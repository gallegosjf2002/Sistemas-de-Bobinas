...
# ------------------------- MODIFICACIÓN DE LA GRÁFICA 3D -----------------------------
# Añadir mapa de calor superpuesto a la gráfica 3D original
fig = plt.figure(figsize=(14, 10))
ax1 = fig.add_subplot(121, projection='3d')
ax2 = fig.add_subplot(122)

# Gráfica 3D original (modificada para temperatura)
surf = ax1.plot_surface(
    X_fine, Y_fine, B_fine,
    facecolors=plt.cm.plasma(B_fine/B_fine.max()),
    rstride=2,
    cstride=2,
    alpha=0.8,
    linewidth=0.2
...
# Añadir texto térmico en 3D
ax1.text2D(0.05, 0.95, 
          f'Potencia térmica por bobina: {potencia_termica:.2f} W\nTemperatura máxima: {solucion.y[0,-1]:.1f}°C',
          transform=ax1.transAxes,
          fontsize=10,
          bbox=dict(facecolor='white', alpha=0.8))

# Gráfica 2D de distribución de temperatura
heatmap = ax2.imshow(
    B_fine,
    extent=[-d, d, -d, d],
    cmap='hot',
...
plt.colorbar(heatmap, label='Densidad de Potencia Térmica (W/m²)')
ax2.set_title('Distribución de Calor en el Plano XY')
ax2.set_xlabel('X (m)')
ax2.set_ylabel('Y (m)')
